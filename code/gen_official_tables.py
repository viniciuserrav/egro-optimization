...
def main():
    ...
    for dim in DIMS:
        ...
        if any(means[f][a] < 0 for f in valid for a in ALGOS):
            note += (r'.  Negative entries indicate the implementation'
                     r"'s stated reference optimum lies above its attainable "
                     r'minimum (official F10); rankings are unaffected, as '
                     r'the reference is a per-function constant')
        lines = [
